...
1. **克隆仓库**
   ```powershell
   git clone <仓库地址>
   cd claude-code-deployment
   ```

2. **下载 CLIProxyAPI 可执行文件**
   - 访问 [CLIProxyAPI Releases](https://github.com/router-for-me/CLIProxyAPI/releases)
   - 下载最新版本的 Windows AMD EXE 程序
   - 将 `cli-proxy-api.exe` 放入 `CLIProxyAPI/` 目录（如果没有该目录请先创建）
