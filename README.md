...
若您不熟悉 Node.js 和命令行，且有 Cursor / Trae 等 AI IDE，可以让 AI 引导您完成安装：

1. 克隆本仓库到本地：
   ```
   git clone https://github.com/baizhichen-web/get_claudecode.git
   ```
2. 用 Cursor / Trae 打开该文件夹
3. 在 AI 对话框中输入以下内容并发送：
   ```
   Help me install Claude Code
   ```
4. AI 会自动分析项目结构，并逐步引导您完成安装（下载 CLIProxyAPI、创建配置、登录授权等），按照提示操作即可。
